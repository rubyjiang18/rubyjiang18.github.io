...
- 设系统一共有$n$个用户，那么$|u_i|=|v_j|=n/10$
- 召回桶$u_i$和召回桶$u_j$的交集为$u_i \bigcap u_j=\varnothing$，即同层互斥，两个召回实验不会同时作用到同一用户上。
- 召回桶$u_i$和精排桶$v_j$的交集为$u_i \bigcap v_j=n/100$。一个用户不能同时被两个召回实验影响，但可以同时受一个召回实验和一个精排实验影响。一个召回实验和一个精排实验各自作用在$n/10$个用户上，那么有$n/100$的用户同时被召回实验和精排实验影响，即不同层正交。
